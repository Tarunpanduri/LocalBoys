...
export default function HomeScreen({ navigation }) {
        ...
        const fetchUserData = async () => {
            try {
                ...
                if (userSnap.exists()) {
                    ...
                    setUserData(u);
                    setUserLocation({ lat: u.location?.lat, lng: u.location?.lng, formattedAddress: u.location?.formattedAddress });
                    setGreetingName(u.firstName || "User");
                }
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const unsubShops = onValue(shopsRef, snap => {
            ...
            console.warn("shops read error", err);
            setLoading(false);
        });

        const unsubNotifs = onValue(notifRef, snap => {
            setNotificationsCount(Object.values(snap.val() || {}).filter(x => !x.isRead).length);
        });
    ...
    useEffect(() => {
        ...
        const filtered = shops
            .map(s => ({ ...s, distanceKm: s.location?.lat && s.location?.lng ? haversineDistance(userLocation.lat, userLocation.lng, Number(s.location.lat), Number(s.location.lng)) : Infinity }))
                        ...
                        ListEmptyComponent={() => (
                            <View style={styles.emptyState}>
                                <Text style={styles.emptytext}>
                                    Sorry, our services are currently unavailable at this location. We will notify when we are here
                                </Text>
                            </View>
                        )}
                    />
                ...
                <View style={{ marginTop: 40, paddingHorizontal: 18 }}>
                    ...
                    <TouchableOpacity style={styles.rideCard}>
                        ...
                        <View style={styles.rideInfo}>
                            ...
                            <Text style={styles.rideDesc}>Taxi, Auto or Shuttle</Text>
                        </View>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity style={styles.rideCard}>
                        <View style={styles.rideInfo}>
                            <Text style={styles.rideTitle}>Rent a Bike</Text>
                            <Text style={styles.rideDesc}>Scooter, Motorcycle</Text>
                        </View>
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            <View style={styles.screen}>
                <View style={styles.headerRow}>
                    <View style={styles.deliveryCol}>
                        ...
                        <TouchableOpacity style={styles.locationRow} onPress={() => navigation.navigate("MapScreen")}>
                            <Text style={styles.locationText}>{userData?.location?.city ? `${userData.location.city}, ${userData.location.state}` : "Fetching..."}</Text>
                ...
                <View style={[styles.bottomNav, { backgroundColor: activeCategoryColor }]}>
                    <TouchableOpacity style={[styles.navButton, activeTab === "ordering" && { backgroundColor: darkenColor(activeCategoryColor, 20) }]} onPress={() => setActiveTab("ordering")}>
                        <Text style={styles.navText}>Ordering</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.navButton, activeTab === "rides" && { backgroundColor: darkenColor(activeCategoryColor, 20) }]} onPress={() => setActiveTab("rides")}>
