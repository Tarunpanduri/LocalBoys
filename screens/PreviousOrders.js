...
export default function PreviousOrders({ navigation }) {
  ...
  const getStatusColor = (status) => {
    ...
    if (s === "completed") return { bg: "#E8F5E9", text: "#2E7D32" }; // Green
    if (s === "cancelled" || s === "rejected") return { bg: "#FFEBEE", text: "#C62828" }; // Red
    return { bg: "#F5F5F5", text: "#616161" }; // Grey fallback
  };
  ...
  const renderOrderCard = ({ item: order }) => {
    ...
    // Count total items
    const itemCount = order.items ? Object.values(order.items).reduce((acc, curr) => acc + (curr.qty || 1), 0) : 0;
      ...
      >
        <View style={styles.cardHeader}>
          <View style={styles.shopInfoWrap}>
            <Image 
              source={order.shopimage ? { uri: order.shopimage } : require('../assets/logo.png')} 
              style={styles.shopImage} 
            />
            <View style={styles.shopTextWrap}>
              <Text style={styles.shopName} numberOfLines={1}>{order.shopname || "Unknown Shop"}</Text>
        ...
        <View style={styles.cardFooter}>
          <Text style={styles.itemCountText}>
            {itemCount} {itemCount === 1 ? 'Item' : 'Items'} • {order.orderType === 'parcel' ? 'Parcel' : 'Delivery'}
          </Text>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
  ...
  const renderFooter = () => {
    ...
    if (hasMorePastOrders && pastOrders.length >= 10) {
      return (
        <TouchableOpacity style={styles.viewMoreBtn} onPress={fetchMorePastOrders}>
          <Text style={styles.viewMoreText}>View More Orders</Text>
        </TouchableOpacity>
      );
    }

    if (!hasMorePastOrders && pastOrders.length > 0) {
      return (
        <View style={styles.footerLoader}>
          <Text style={styles.endOfListText}>You've reached the end of your order history.</Text>
        </View>
      );
    }
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalScrollContent}>
              ...
              <View style={styles.modalShopRow}>
                <Image source={selectedOrder.shopimage ? { uri: selectedOrder.shopimage } : require('../assets/logo.png')} style={styles.modalShopImage} />
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Text style={styles.modalShopName}>{selectedOrder.shopname || "Unknown Shop"}</Text>
              ...
              {selectedOrder.discount > 0 && (
                <View style={styles.billRow}>
                  ...
                  <Text style={[styles.billValue, { color: '#2E7D32' }]}>- ₹{selectedOrder.discount}</Text>
                </View>
              )}

              <View style={styles.billRow}>
                <Text style={styles.billLabel}>Delivery Fee</Text>
                <Text style={styles.billValue}>₹{selectedOrder.deliveryFee}</Text>
              </View>

              <View style={styles.billRow}>
                <Text style={styles.billLabel}>Platform Fee</Text>
                <Text style={styles.billValue}>₹{selectedOrder.platformFee || 0}</Text>
              </View>

              <View style={[styles.billRow, styles.grandTotalRow]}>
                <Text style={styles.grandTotalLabel}>Paid via {selectedOrder.paymentMode}</Text>
              ...
              <View style={styles.addressBox}>
                ...
                <View style={{ marginLeft: 10, flex: 1 }}>
                  {selectedOrder.orderType === 'parcel' ? (
                    <>
                      <Text style={styles.addressType}>Drop Address</Text>
                      <Text style={styles.addressText}>{selectedOrder.dropAddress?.formattedAddress || "N/A"}</Text>
                    </>
                  ) : (
                    <>
                      <Text style={styles.addressType}>Delivery Address</Text>
                      <Text style={styles.addressText}>{selectedOrder.address || selectedOrder.deliveryAddress?.formattedAddress || "N/A"}</Text>
                    </>
                  )}
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  receiptItemName: { fontSize: 14, fontFamily: 'Sen_Medium', color: '#333', flex: 1 },
