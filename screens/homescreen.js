...
const SkeletonItem = ({ width: w, height, style, borderRadius = 4 }) => {
  ...
  return (
    <View
      style={[
        { width: w, height, backgroundColor: "#E1E9EE", borderRadius, overflow: "hidden" },
        style,
      ]}
    >
      <Animated.View
  ...
  <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.mediumcontent}>
        ...
        {/* Skeleton Banner - uses negative margin since its parent (mediumcontent) has 18px padding */}
        <SkeletonItem
          width={width}
          height={width * 0.5}
          borderRadius={0}
...
// CATEGORY CHIPS ROW
// ─────────────────────────────────────────────
const CategoryChipsRow = React.memo(
  ({ categories, activeCategory, onPress, categoryMeta }) => (
    <ScrollView
      horizontal
      ...
      {categories.map((item) => {
        ...
        return (
          <TouchableOpacity
            ...
            style={[
              styles.categoryChip,
              active && { backgroundColor: darkenedColor, borderColor: darkenedColor },
            ]}
          >
            <Ionicons
              name={iconName}
            ...
            <Text
              style={[
                ...
                active && { color: "#fff", fontFamily: "Sen_Bold" },
              ]}
            >
              {item.label}
            </Text>
    ...
    <View style={styles.shopInfo}>
      ...
      <View style={styles.shopMetaRow}>
        <View style={styles.metaItem}>
          <FontAwesome5 name="star" size={14} />
        ...
        <View style={[styles.metaItem, { marginLeft: 12 }]}>
          ...
          <Text style={styles.metaText}>
            {" "}
            {shop.deliveryTime ?? `${shop.avgPrepTime ?? "—"} min`}
          </Text>
        </View>
      </View>
    </View>
...
export default function HomeScreen({ navigation }) {
  ...
  const categoryRowBottomRef = useRef(300);
  const stickyBarVisibleRef = useRef(false);
  ...
  const handleShopPress = useCallback(
    (shopId, shop) => navigation.navigate("ShopDetails", { shopId, shop }),
    [navigation]
  );
  ...
  useEffect(() => {
    if (!hasValidLocation && !userLoading) {
      const t = setTimeout(() => addressesSheetRef.current?.expand(), 300);
  ...
  const { filteredShops, dynamicCategories } = useMemo(() => {
    ...
    let result = shops.filter(
      (s) => s.isActive !== false && activeBranchIds.includes(s.parentBranchId)
    );
    ...
    if (searchText) {
      ...
      shopsForDisplay = shopsForDisplay.filter(
        (s) =>
    ...
    const shopTypes = Array.from(new Set(result.map((s) => s.type?.trim())))
      .filter(Boolean)
      .map((t) => ({ id: t.toLowerCase(), label: t }));

    return {
      filteredShops: shopsForDisplay,
      dynamicCategories: [{ id: "all", label: "All" }, ...shopTypes],
    };
  ...
  const onRefresh = useCallback(async () => {
    if (!userLocation?.lat || !userLocation?.lng || activeBranchIds.length === 0) return;
    setRefreshing(true);
    determineBranches(userLocation.lat, userLocation.lng, userLocation.pincode);
  ...
  const activeCategoryColor =
    categoryMeta[dynamicCategories.find((c) => c.id === activeCategory)?.label]?.Theme ||
    "#66BB6A";

  const handleCategoryPress = useCallback((id) => {
  ...
  const handleScroll = useCallback((event) => {
    ...
    const shouldShow = y > categoryRowBottomRef.current;
    if (shouldShow !== stickyBarVisibleRef.current) {
      stickyBarVisibleRef.current = shouldShow;
  ...
  const onCategoryRowLayout = useCallback((event) => {
    ...
    categoryRowBottomRef.current = y + height - 30;
  }, []);
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
          ...
          <View style={styles.noAddressContainer}>
            ...
            <Text style={styles.noAddressSub}>
              Please add an address to see shops and services near you.
            </Text>
            <TouchableOpacity
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
        ...
        <LinearGradient
          colors={[activeCategoryColor || "#66BB6A", "#ffffff"]}
          start={{ x: 0, y: 0 }}
          end={{ x: 0, y: 1 }}
          style={{ position: "absolute", top: 0, left: 0, right: 0, height: 180 }}
        />
        ...
        <View style={styles.headerRow}>
          <View style={styles.deliveryCol}>
            <Text style={[styles.deliverLabel, { color: darkenColor(activeCategoryColor, 40) }]}>
              Deliver To
            </Text>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View>
              ...
              {/* Event Banner (full-bleed, no horizontal padding) */}
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <Text style={styles.modalMessage}>
                You're currently browsing as a guest. To {modalFeatureText}, please log in or
                create a free account with us to have hassle-free access.
              </Text>
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  screen: { flex: 1 },
  mediumcontent: { flex: 1, paddingHorizontal: 18 },
  ...
  scrollHeaderContainer: { paddingHorizontal: 18, paddingTop: 10 },
  shopCardWrapper: { paddingHorizontal: 18 },
